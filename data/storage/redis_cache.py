...
class RedisCache:
    ...
    async def publish_tick(self, symbol: str, tick: dict[str, Any]) -> None:
        ...
        assert self._client, "call initialize() first"
        await self._client.publish(_TICK_CH.format(symbol=symbol), json.dumps(tick, default=str))
    ...
    async def publish_candle(self, symbol: str, candle: dict[str, Any]) -> None:
        ...
        assert self._client, "call initialize() first"
        await self._client.publish(_CANDLE_CH.format(symbol=symbol), json.dumps(candle, default=str))

    async def publish_signal(self, symbol: str, signal: dict[str, Any]) -> None:
        """Publish a trading signal to the signals:{symbol} channel."""
        assert self._client, "call initialize() first"
        await self._client.publish(_SIGNAL_CH.format(symbol=symbol), json.dumps(signal, default=str))
    ...
    async def publish_dqg_status(self, symbol: str, report: dict[str, Any]) -> None:
        ...
        assert self._client, "call initialize() first"
        await self._client.publish(_DQG_CH.format(symbol=symbol), json.dumps(report, default=str))
